...
from backend.models.schemas import AnalysisResponse, ComponentScores, JDComparison
...
async def analyze_resume(
    ...
    # Convert JD comparison result to API schema
    ...
    if result.get('jd_comparison'):
        jd_comparison_result = JDComparison(
            ...
            matched_skills=result['jd_comparison'].get(
                'matched_skills', []
            )[:15],
            missing_skills=result['jd_comparison'].get(
                'missing_skills', []
            )[:15],
    ...
    response = AnalysisResponse(
        ...
        jd_recommendations=result.get('jd_recommendations', []),
...
async def generate_pdf(
    ...
    try:
        ...
        analysis_data["candidate_name"] = analysis_data.get(
            "candidate_name", ""
        )
        analysis_data["email"] = analysis_data.get("email", "")
        analysis_data["phone"] = analysis_data.get("phone", "")
